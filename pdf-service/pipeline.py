    ...
    try:
        ...
        with ai_client.use_config(ai_config):
            if kernel_pdf_kind == "scanned_question_book":
                kernel_result = parse_extractor_with_kernel(
                    extractor,
                    debug_dir=debug_dir,
                )
    ...
    for fallback_index, raw_question in enumerate(result.get("questions", []), start=1):
        ...
        questions.append(
            Question(
                ...
                source_text_span=raw_question.get("source_text_span"),
                source_anchor_text=raw_question.get("source_anchor_text"),
                source_confidence=raw_question.get("source_confidence"),
                material_group_id=raw_question.get("material_group_id"),
                material_group_question_indexes=raw_question.get("material_group_question_indexes") or [],
                material_group_confidence=raw_question.get("material_group_confidence"),
                material_group_reason=raw_question.get("material_group_reason"),
                shared_material=bool(raw_question.get("shared_material")),
